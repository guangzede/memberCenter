    ...
    <div class="app-container shadow">
      <div class="content">
        ...
        <el-table :data="tableData" style="width: 100%" stripe >
          ...
          <!--              {{ scope.row.integral>=0?'成长值增加':'成长值减少' }}-->
          <!--            </template>-->
          <!--          </el-table-column>-->
          <el-table-column label="手机号" ><template slot-scope="scope">{{ scope.row.phone?scope.row.phone:'-' }}</template></el-table-column>
          <el-table-column label="变动成长值"><template slot-scope="scope">{{ scope.row.integral?scope.row.integral:'-' }}</template></el-table-column>
          ...
          <el-table-column label="变动后成长值总值"><template slot-scope="scope">{{ scope.row.last_total_growth?scope.row.last_total_growth:'-' }}</template></el-table-column>
          <el-table-column label="成长值变动说明"><template slot-scope="scope">{{ scope.row.vremark?scope.row.vremark:'-' }}</template></el-table-column>
        </el-table>
...
<script>
import { getGrowthSteamBrand, exportGrowthListExcel, getChoiseListForPG } from '@/api/growth'
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      changeType: [
        { label: '成长值增加', value: 'integral_add' },
        { label: '成长值扣减', value: 'integral_minus' }
      ],
      tRemarkAndBId: [],
      choise: 'growth'
    }
  ...
  methods: {
    exportExcelHandler() {
      ...
      if (this.count > 10000) {
        this.$confirm('当前查询的数据量【已超过10000条】，导出时响应时间较长，您是否确定导出数据？', '提示信息', {
          ...
          exportGrowthListExcel(param).then(res => {
            // const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
            const blob = new Blob([res])
            const dt = new Date()
            const ex_name = '成长值流水' + dt.getFullYear() + (dt.getMonth() + 1) + dt.getDate() + dt.getHours() + dt.getMinutes() + dt.getSeconds() + '.xlsx'
    ...
    init(pageNo) {
      ...
      getGrowthSteamBrand(param).then(res => {
